...
extern int speed;  // ✅ Just refer to the speed from the .ino file
int noStop = 0;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  // Lock manual controls if autonomous mode is active
extern bool autonomous;
if (autonomous) {
  if (strcmp(variable, "speed") != 0 && strcmp(variable, "nostop") != 0) {
    Serial.println("Manual control blocked: robot is in autonomous mode");
    httpd_resp_set_type(req, "text/plain");
    return httpd_resp_send(req, "Manual control disabled in autonomous mode", HTTPD_RESP_USE_STRLEN);
  }
}
...
// Look at values within URL to determine function
  ...
  autonomous = (val == 1);
  Serial.printf("Autonomous mode set to: %s\n", autonomous ? "ON" : "OFF");
}
...
<html>
    ...
    <br/>
          ...
          <section id="buttons">
                ...
                <div id="controls" class="control-container">
                  <table>
                  <tr><td align="center"><button class="button button6" id="get-still">Image</button></td><td align="center"><button id="toggle-stream" onclick="fetch(document.location.origin+'/control?var=mode&val=1')">Start</button>
</td><td></td></tr>
  ...
  <td align="center" colspan="3">
    <button class="button button5" onclick="fetch(document.location.origin+'/control?var=mode&val=1')">GO AUTONOMOUS</button>
  </td>
</tr>
